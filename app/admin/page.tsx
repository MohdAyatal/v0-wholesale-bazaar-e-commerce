...
export default function AdminDashboard() {
  ...
  const handleEditProduct = (product: Product) => {
    setFormData({ name: product.name, price: product.price, category_id: product.category_id, image_urls: product.image_urls || [], discount_percent: product.discount_percent || 0, description: '' })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex gap-4 mb-8 border-b overflow-x-auto" style={{ borderColor: 'var(--border)' }}>
          {(['products', 'slideshow', 'orders'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ borderBottomColor: tab === t ? 'var(--primary)' : 'transparent', color: tab === t ? 'var(--primary)' : 'var(--text-secondary)' }}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="p-6 rounded-lg border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
            ...
            <p className="text-3xl font-bold mt-2" style={{ color: 'var(--primary)' }}>{stats.totalProducts}</p>
          </div>
          <div className="p-6 rounded-lg border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Total Orders</p>
            <p className="text-3xl font-bold mt-2" style={{ color: 'var(--secondary)' }}>{stats.totalOrders}</p>
          </div>
          <div className="p-6 rounded-lg border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Total Revenue</p>
            <p className="text-3xl font-bold mt-2" style={{ color: 'var(--accent)' }}>₹{stats.totalRevenue.toLocaleString()}</p>
          </div>
        </div>
        ...
        {tab === 'products' && (
          <>
            <div className="mb-6">
              <button onClick={() => setShowModal(true)} className="flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-white transition hover:opacity-90" style={{ backgroundColor: 'var(--primary)' }}>
                  ...
                  <tbody>
                    {products.map((product) => (
                      <tr key={product.id} className="border-t hover:opacity-80 transition" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--background)' }}>
                        ...
                        <td className="px-6 py-4" style={{ color: 'var(--text-secondary)' }}>{categories.find(c => c.id === product.category_id)?.name || 'N/A'}</td>
          ...
          <>
            <div className="mb-6">
              <button onClick={() => { setEditingId(null); setSlideFormData({ title: '', description: '', media_url: '', type: 'image', order: 0 }); setShowModal(true) }}
                className="flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-white transition hover:opacity-90" style={{ backgroundColor: 'var(--primary)' }}>
              ...
              {slides.map((slide) => (
                <div key={slide.id} className="rounded-lg border overflow-hidden hover:shadow-lg transition" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
                  <div className="relative h-40 flex items-center justify-center" style={{ backgroundColor: 'var(--background)' }}>
                    {slide.type === 'image' ? <ImageIcon size={40} style={{ color: 'var(--primary)', opacity: 0.3 }} /> : <Play size={40} style={{ color: 'var(--primary)', opacity: 0.3 }} />}
                  </div>
                  <div className="p-4">
            ...
            <div className="rounded-lg border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {stats.totalOrders === 0 ? (
                      <tr><td colSpan={7} className="px-6 py-8 text-center" style={{ color: 'var(--text-secondary)' }}>No orders yet.</td></tr>
                    ) : (
                      <tr><td colSpan={7} className="px-6 py-8 text-center">
                        <a href="/admin/orders" className="px-6 py-2 rounded-lg font-medium text-white transition hover:opacity-90" style={{ backgroundColor: 'var(--primary)' }}>View All Orders</a>
                      </td></tr>
                    )}
                  </tbody>
                </table>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="rounded-xl p-8 max-w-md w-full" style={{ backgroundColor: 'var(--surface)' }}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={closeModal} className="p-1"><X size={24} style={{ color: 'var(--text-secondary)' }} /></button>
            </div>
            <div className="space-y-4">
              {tab === 'products' ? (
                <>
                  ...
                  <div>
                    <label className="block font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Image URL</label>
                    <input type="text" onChange={(e) => setFormData({ ...formData, image_urls: [e.target.value] })} className="w-full px-4 py-2 rounded-lg border outline-none" style={{ borderColor: 'var(--border)' }} placeholder="https://..." />
                  </div>
                  <div>
                    <label className="block font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Description</label>
                    <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-2 rounded-lg border outline-none" style={{ borderColor: 'var(--border)' }} rows={3} placeholder="Enter product description" />
                  </div>
                ...
                <>
                  ...
                  <div>
                    <label className="block font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Description</label>
                    <textarea value={slideFormData.description} onChange={(e) => setSlideFormData({ ...slideFormData, description: e.target.value })} className="w-full px-4 py-2 rounded-lg border outline-none" style={{ borderColor: 'var(--border)' }} rows={3} placeholder="Enter slide description" />
                  </div>
                  <div>
